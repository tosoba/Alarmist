...
import com.trm.alarmist.core.domain.usecase.calculateMissedAlarmsDateTimes
...
class CalculateMissedAlarmsDateTimesUseCaseTests {
  ...
  fun `given empty alarms - then return emptyMap`() = runTest {
    assertEquals(expected = emptyMap(), actual = calculateMissedAlarmsDateTimes(emptyList()))
  }
  ...
  fun `given only alarms with null lastNotificationDate that were not missed - then return emptyMap`() =
    runTest {
      ...
      assertEquals(
        expected = emptyMap(),
        actual =
          calculateMissedAlarmsDateTimes(
            listOf(
              alarmModel(
  ...
  fun `given one time alarms one of which was missed - then return missed alarm with single dateTime`() =
    runTest {
      ...
      assertEquals(
        ...
        actual = calculateMissedAlarmsDateTimes(alarms),
      )
    }

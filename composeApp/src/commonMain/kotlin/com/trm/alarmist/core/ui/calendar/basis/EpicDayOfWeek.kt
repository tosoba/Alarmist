...
fun daysOfWeekSortedBy(firstDayOfWeek: DayOfWeek) =
  DayOfWeek.entries.toTypedArray().let {
    val n = it.size - firstDayOfWeek.ordinal
    it.takeLast(n) + it.dropLast(n)
  }

fun DayOfWeek.index(firstDayOfWeek: DayOfWeek = firstDayOfWeek()) =
  when (firstDayOfWeek) {
    DayOfWeek.MONDAY -> isoDayNumber - 1
    DayOfWeek.SUNDAY -> {
      if (this == DayOfWeek.SUNDAY) 0 else isoDayNumber
    }

...
internal fun DrawScope.drawEpicRange(
  ...
  if (rangeInfo.isStartInGrid) {
    translate(left = startX - (itemSize.width / 2), top = startY) {
      drawOutline(outline = itemShapeOutline, color = color)
    }
  }

  if (rangeInfo.isEndInGrid) {
    translate(left = endX - (itemSize.width / 2), top = endY) {
      drawOutline(outline = itemShapeOutline, color = color)
    }
  }
  ...
  if (y1 != y2) {
    for (y in y2 - y1 - 1 downTo 1) {
      drawRect(
        color = color,
        topLeft = Offset(x = 0f, y = startY + y * (itemSize.height + rowsSpacerHeightPx)),
        size = Size(width = size.width, height = itemSize.height),
      )
    }

    drawRect(
      color = color,
      topLeft = Offset(x = 0f, y = endY),
      size = Size(width = endX, height = itemSize.height),
    )
  }
}
